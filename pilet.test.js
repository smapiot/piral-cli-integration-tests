...
const waitForRunning = (debugProcess, port) => {
  return new Promise((resolve, reject) => {
    timeout = setTimeout(
      () => reject(new Error("Server not started after 180s")),
      180 * 1000
    );
    const ref = {};
...
describe("pilet", () => {
  it("scaffold pilet", async () => {
    ...
    const info = await execute(
      `npm init pilet@${cliVersion} ${installFlag}-y`,
      {
  ...
  it("HMR", async (done) => {
    ...
    const debugProcess = spawn(
      `${timeoutCommand}npx pilet debug ${
    ...
    const backendReloaded = new Promise((resolve, reject) => {
      timeout = setTimeout(
        () => reject(new Error("Server not started after 60s")),
        60 * 1000
      );
    ...
    await writeFile(
      srcFilePath,
      indexFile.toString().replace("Welcome to Piral!", newString)
    );
